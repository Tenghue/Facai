...
def load_group_data(filename='all3D.csv'):
...
def main():
    ...
    try:
        # =============== 1. 加载原始数据 ===============
        periods, raw_codes = load_group_data('all3D.csv')
        if not periods:
            print("❌ 无有效数据")
            return
        ...
        print(f"\n✅ 分析完成！已分析所有0-9数字的遗漏期高频组合特征。")
                      ...
                      # =============== 8. 新增：全局遗漏期高频单码 Top 4 统计 ===============
        print(f"\n{'='*85}")
        print(f"🏆 全局遗漏期高频单码 Top 4 统计（基于偏离度降序）")
        print(f"{'='*85}")
        # 收集所有数字分析结果中的排名第一的单码
        global_top_digits = []
        for digit, result in all_results.items():
            stats = result.get('digit_stats', [])
            if stats:  # 确保有统计数据
                top1 = stats[0]  # digit_stats 已经是按偏离度排好序的，取第一个
                global_top_digits.append({
                    '触发数字': digit,
                    '高频单码': top1['单码'],
                    '遗漏期频率(%)': top1['遗漏期频率(%)'],
                    '偏离度(倍)': top1['偏离度(倍)'],
        ...
        # 按照偏离度（倍）进行全局降序排序
        global_top_digits.sort(key=lambda x: x['偏离度(倍)'], reverse=True)
        # 动态提取并打印排名前四位（Top 4）的单码
        top_n = min(4, len(global_top_digits))  # 防止数据不足4个时报错
        if top_n > 0:
            # 定义排名对应的奖牌 Emoji
            medals = ["🥇", "🥈", "🥉", "4️⃣"] 
            for rank in range(top_n):
                item = global_top_digits[rank]
                print(f"{medals[rank]} 排名第 {rank + 1} 的高频单码: {item['高频单码']} "
                      f"(触发数字: {item['触发数字']}, "
                      f"偏离度: {item['偏离度(倍)']}倍, "
                      f"遗漏期频率: {item['遗漏期频率(%)']}%, "
                      f"状态: {item['状态']})")
        else:
            print("⚠️ 数据不足，未能统计出高频单码 Top 4。")
