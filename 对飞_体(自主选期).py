...
def filter_data_by_end_period(df, end_period):
    """根据截止期号过滤数据"""
    # 确保截止期号存在于数据中
    if end_period not in df['期号'].values:
        available_periods = df['期号'].tolist()
        closest_period = None
        # 寻找小于等于输入期号的最大期号
        for period in reversed(available_periods):
            if safe_sort_key(period) <= safe_sort_key(end_period):
                closest_period = period
                break
        
        if closest_period is None:
            raise ValueError(f"❌ 输入的期号 {end_period} 太小，数据中最小期号为 {df['期号'].iloc[0] if len(df) > 0 else 'N/A'}")
        else:
            print(f"⚠️  期号 {end_period} 不在数据中，使用最接近的期号 {closest_period} 作为截止期")
            end_period = closest_period
    
    # 截取数据到指定期号
    end_idx = df[df['期号'] == end_period].index[0]
    filtered_df = df.iloc[:end_idx + 1].copy().reset_index(drop=True)
    
    return filtered_df, end_period
...
def generate_report_and_save(stats, original_df, end_period):
    ...
    lines.append(f"🔍 截止期号: {end_period}")
    lines.append(f"🔍 数据范围: {original_df['期号'].iloc[0]} → {end_period} | 实际分析期数: {len(original_df[original_df['期号'] <= end_period])}")
    lines.append("💡 核心定义:")
    ...
    # 保存报告
    report = "\n".join(lines)
    txt_file = f'GroupThree_DigitMiss_Report_{end_period}_{timestamp}.txt'
    with open(txt_file, 'w', encoding='utf-8') as f:
        f.write(report)
    
    # 保存明细到CSV
    detail_df = pd.DataFrame(sorted_stats)
    csv_file = f'GroupThree_DigitMiss_Details_{end_period}_{timestamp}.csv'
...
def main():
    ...
    try:
        ...
        # 新增：获取用户输入的截止期号
        print(f"📊 数据中可用的期号范围: {df['期号'].iloc[0]} → {df['期号'].iloc[-1]}")
        end_period_input = input("请输入截止期号 (直接回车使用最新期): ").strip()
        
        if not end_period_input:
            end_period = df['期号'].iloc[-1]
            print(f"✅ 使用最新期号: {end_period}")
        else:
            end_period = end_period_input
        
        # 过滤数据到指定截止期
        filtered_df, final_end_period = filter_data_by_end_period(df, end_period)
        print(f"✅ 已过滤数据至期号: {final_end_period}，共 {len(filtered_df)} 期")
        
        stats = analyze_group_three_digits(filtered_df)
        generate_report_and_save(stats, df, final_end_period)
