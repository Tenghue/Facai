# 文件名：pl3_baozi_leakage_advanced_with_position.py
import pandas as pd
import numpy as np
import sys
import os
...
def analyze_single_digit(digit, csv_path, period_col=None, number_col=None):
    """
    新增功能：统计指定数字的所有豹子号后，再次出现为独码的遗漏值列表，并统计位置
    """
    ...
    print("-" * 100)
    print(f"{'豹子号期号':<12} {'再次出现期号':<15} {'遗漏值':<8} {'再次出现号码':<12} {'位置':<20}")
    print("-" * 100)

    gaps_list = []
    positions_list = [] # 新增：记录位置
    all_positions_stats = {'百位': 0, '十位': 0, '个位': 0} # 新增：位置统计
        ...
        for j in range(baozi_idx + 1, len(df)):
            ...
            if str(digit) in num_str:
                ...
                for idx, char in enumerate(num_str):
                    if char == str(digit):
                        if idx == 0:
                            pos_details.append('百位')
                            all_positions_stats['百位'] += 1
                        elif idx == 1:
                            pos_details.append('十位')
                            all_positions_stats['十位'] += 1
                        elif idx == 2:
                            pos_details.append('个位')
                            all_positions_stats['个位'] += 1
                
                position_str = ', '.join(pos_details)
                
                print(f"{baozi_period:<12} {reappear_period:<15} {gap:<8} {reappear_code:<12} {position_str:<20}")
                
                gaps_list.append(gap)
                positions_list.extend(pos_details) # 记录本次出现的所有位置
                found = True
                break
        
        if not found:
            print(f"{baozi_period:<12} {'未再出现':<15} {'N/A':<8} {'N/A':<12} {'N/A':<20}")
            # 不计入统计列表，因为没有再次出现
    ...
    print(f"  - 位置统计: 百位={all_positions_stats['百位']}, 十位={all_positions_stats['十位']}, 个位={all_positions_stats['个位']}")
