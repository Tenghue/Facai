...
def main():
    ...
    try:
        ...
        # =============== 7. 详细输出（选3个最具代表性数字） ===============
        valid_digits = [d for d, r in all_results.items() if r['total_miss'] >= 10]
        if valid_digits:
            # 按单码最大偏离度排序选3个
            top_digits = sorted(
                valid_digits,
                key=lambda d: max(r['偏离度(倍)'] for r in all_results[d]['digit_stats']) if all_results[d]['digit_stats'] else 0,
                reverse=True
            )[:3]
            
            print(f"\n💡 重点展示「数据充足」且「单码偏离度最高」的3个数字详情（分析截止: {periods[-1]}）")
            for digit in top_digits:
                res = all_results[digit]
                print(f"\n{'─'*85}")
                print(f"🔢 数字 {digit} | 连续事件: {res['event_count']} 次 | 总遗漏期数: {res['total_miss']} 期")
                print(f"\n【高频单码 Top5】")
                print(f"{'单码':<6}{'遗漏期出现':<12}{'遗漏期频率':<12}{'全历史频率':<12}{'偏离度':<10}{'状态'}")
                for r in res['digit_stats']:
                    print(f"{r['单码']:<6}{r['遗漏期出现期数']:<12}{r['遗漏期频率(%)']:<12}{r['全历史频率(%)']:<12}{r['偏离度(倍)']:<10}{r['状态']}")
                
                print(f"\n【高频二码组合 Top8】")
                print(f"{'二码':<8}{'遗漏期出现':<12}{'遗漏期频率':<12}{'全历史频率':<12}{'偏离度':<10}{'状态'}")
                for r in res['combo_stats']:
                    print(f"{r['二码组合']:<8}{r['遗漏期出现期数']:<12}{r['遗漏期频率(%)']:<12}{r['全历史频率(%)']:<12}{r['偏离度(倍)']:<10}{r['状态']}")
        
        # =============== 8. 导出选项 ===============
        if input(f"\n💾 导出完整分析结果到 '遗漏期分析_{periods[-1]}.csv'? (y/n): ").strip().lower() == 'y':
            export_rows = []
            for digit in [str(d) for d in range(10)]:
            ...
            pd.DataFrame(export_rows).to_csv(f'遗漏期分析_{periods[-1]}.csv', index=False, encoding='utf-8-sig')
            print(f"✅ 已导出: 遗漏期分析_{periods[-1]}.csv（含所有数字详细统计）")
        
        print("\n" + "="*85)
        print("🛡️ 核心保障说明:")
        print("  • 所有统计严格基于「截止期号及之前」的历史数据")
        print("  • 截止期号当期数据已包含在分析基准中")
        print("  • 无任何未来数据污染，可安全用于策略回测")
        print("💡 应用建议: 重点关注「偏离度>1.5倍」的单码/二码组合，结合当前遗漏状态综合判断")
        print("="*85)
